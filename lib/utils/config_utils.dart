...
class LocalConfigStorage {
  ...
  void init(String dataFile, Map<String, dynamic> data) {
    ...
    if (!existsDataFile() || !existsDefaultsFile()) {
      ...
      throw Exception(
          "Unable to find file $_dataFilePath or $_defaultsFilePath");
      // _dataMap.clear();
  ...
  void _deleteDataFile() =>
      existsFile(_dataFilePath) ? File(_dataFilePath).deleteSync() : {};
  ...
  Map<String, dynamic> _defaultsFromFile() {
    if (existsFile(_defaultsFilePath) &&
        fileContent(_defaultsFilePath).isNotEmpty) {
  ...
  bool _existsIn(Map<String, dynamic> data, String key) =>
      data.entries.where((e) => e.key == key).isNotEmpty;
